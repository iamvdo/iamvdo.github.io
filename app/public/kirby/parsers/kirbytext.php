...
class kirbytext {
  ...
  function image($params) {
    ...
    // VDO add gd to get width an height
    //$imagick = getimagesize($imageMeta['url']);

    $imageAttributes = $this->attr(array(
      'src'    => $imageMeta['url'],
      //'width'  => @$params['width'] | $imagick[0], 
      //'height' => @$params['height'] | $imagick[1], 
      'class'  => @$params['class'], 
      'title'  => html($title), 
      'alt'    => html($alt)
    ));
    ...
    if(!empty($params['text'])) {
      $legend = '<figcaption class="caption">' . $params['text'] . '</figcaption>';
      return '<figure>' . $image . $legend . '</figure>';
    }
