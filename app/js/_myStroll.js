 ...
 * MIT licensed
 *
 * Forked and updated by Vincent De Oliveira
 *
...
(function(){
	...
	List.prototype.getItems = function() {
		...
		for (var i = 0; i < this.element.children.length; i++) {
			if ( this.element.children[i].classList.contains('Greeed') ){
				var childrens = Array.prototype.slice.apply( this.element.children[i].children );

				for (var j = 0; j < childrens.length; j++) {
					var childrensChild = Array.prototype.slice.apply( childrens[j].children );
					items = items.concat(childrensChild);
				}
				
			}
		}
		*/
		var itemsNodeList = document.querySelectorAll('.item');
		for (var i = 0; i < itemsNodeList.length; i++) {
			items.push(itemsNodeList[i]);
		};
	...
	TouchList.prototype.onTouchEnd = function( event ) {
		...
		if( distanceMoved === 0 ) {
			...
			if ( event.target instanceof HTMLAnchorElement ) {
				...
				location = event.target.parentElement.href;
			}
